...
For the pedestrian detection using Detectron2, the miss rate for 2000 Kb/s was 0.91, for 1000 Kb/s - 0.83, for 500 Kb/s - 0.74. The relatively highest number of false positives was observed in the scene where the image degradation was the highest (see Figure \ref{fig7}).
...
In the case of the Hog descriptor, the miss rate dropped from 0.97 for 2000kbps to 0.81 for 500kbps. However, the fall is not as strong as for pedestrian detection, which is primarily related to the background but the video in the dataset (the tone of the clothes of the people is dark and matches the tone of the walls and shadows).

As a result, even such a powerful neural network as Detector2 did not detect a large number of pedestrians at a bitrate of 500kbps compared to 4000kbps.

Such modern models trained on high quality image datasets make unreliable predictions when they encounter compression artifacts in the input due to their inability to generalize from their exact training sets. To create object detectors that are more robust to these degradations, it is necessary either to supplement their dataset with additional images with a reduced bitrate (although then the efficiency of Detectron2 on a high-bitrate video detector2 may decrease), or to apply additional video modifications, which we will write about later.
